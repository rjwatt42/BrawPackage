reRangeY<-function(y) {
  ...
  y<-y*(braw.env$plotArea[4]-0.25)+braw.env$plotArea[2]+0.25
  return(y)
}
reRangeX<-function(x) {
  if (is.null(braw.env$plotLimits)) return(x)
  x<-(x-braw.env$plotLimits$xsc[1])/diff(braw.env$plotLimits$xsc)
  x<-x*(braw.env$plotArea[3]-0.25)+braw.env$plotArea[1]+0.25
  return(x)
}
...
plotLimits<-function(xlim,ylim,orientation) {
  switch(orientation,
         "horz"={braw.env$plotLimits<-list(xlim=xlim,ylim=ylim,xsc=xlim,ysc=ylim,orientation=orientation)},
         "vert"={braw.env$plotLimits<-list(xlim=xlim,ylim=ylim,xsc=ylim,ysc=xlim,orientation=orientation)}
         )
}

startPlot<-function(xlim,ylim,box="both",orientation="horz",g=NULL) {
  ...
  g<-g+dataPolygon(data=back,fill=braw.env$plotColours$graphBack, colour=braw.env$plotColours$graphBack)
  switch(box,
         "x"={
...
xAxisLabel<-function(label) {
  ...
  switch(braw.env$plotLimits$orientation,
         ...
         "horz"={
           geom_text(data=axis,aes(x=x,y=y),label=deparse(label), parse = TRUE,hjust=0.5,vjust=2.4,
