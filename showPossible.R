...
showPossible <- function(possible=makePossible(),possibleResult=NULL,
                         cutaway=FALSE,
                         view="3D",show="Normal",scale=1,
                         azimuth=50,elevation=5,range=2,boxed=FALSE){
  
  if (is.null(possibleResult)) possibleResult<-runPossible(possible)
  BoxCol<-"#666666"
  ...
  if (show=="Power") {
    ...
    sampleLikelihood_r<-possibleResult$Theory$spDens_w
  ...
  offRange<-0
  if (possible$type=="Samples" && scale<1 && !is.na(possible$targetSample)) offRange<-possible$targetSample  
  if (possible$type=="Populations" && scale<1 && !is.na(possible$targetSample)) offRange<-possible$targetSample  
  if (braw.env$RZ=="r") {
    xlim<-c(-1,1)*scale+offRange # population
    ylim<-c(-1,1)*scale+offRange
  } else {
    xlim<-c(min(rp),max(rp))*scale+offRange # population
    ylim<-c(min(rs),max(rs))*scale+offRange
  }
  zlim<-c(0,1)
  if (show=="Power") {
    if (w_range[1]<0.5) {
  ...
  switch (view,
          "3D"= {
            ...
            mapping<-persp(xlim,ylim,z, 
                           ...
                           zlim = zlim, 
                           theta = azimuth, phi = elevation, r=range, 
                           ticktype = "simple", 
                           box = FALSE,
                           axes = FALSE,
            ...
            # x and y ticks
              ...
              if (show=="Power" && possible$type=="Populations") {
            ...
            text(ticks.x$x,ticks.x$y,long_ticks,cex=0.6*char3D,adj=c(1,NA))
              ...
              if (show=="Power" && possible$type=="Samples") {
                      ...
                      for (i in order(sourceRVals)) {
                        ...
                        if (!isempty(sRho)){
                          for (si in 1:length(sRho)) {
                            ...
                            lines(trans3d(x=c(sourceRVals[i],sourceRVals[i]),y=c(sRho[si],sRho[si]),z=c(zlim[1],z),pmat=mapping),col=colVline, lwd=1)
                            # connecting lines
  ...
  "2D"={
    ...
    switch (possible$type,
            "Populations"={
              ...
              rw_dens<-rpw_dens
              if (braw.env$RZ=="z") {
                if (show=="Power") {xlabel<-bquote(bold(w['p']))} else {xlabel<-bquote(bold(z['p']))}
              } else {
                if (show=="Power") {xlabel<-bquote(bold(w['p']))} else {xlabel<-bquote(bold(r['p']))}
              }
              col<-colP
              },
            "Samples"={
              ...
              rw_dens<-rsw_dens
              if (braw.env$RZ=="z") {
                if (show=="Power") {xlabel<-bquote(bold(w['s']))} else {xlabel<-bquote(bold(z['s']))}
              } else {
                if (show=="Power") {xlabel<-bquote(bold(w['s']))} else {xlabel<-bquote(bold(r['p']))}
              }
              col<-colS
            }
    )
